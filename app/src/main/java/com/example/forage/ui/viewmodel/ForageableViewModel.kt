...
class ForageableViewModel(private val forageableDao: ForageableDao): ViewModel() {
    var fullForageable: LiveData<List<Forageable>> = forageableDao.getForageables().asLiveData()
    fun forageableId(id: Long): LiveData<Forageable> {
...
class ForageableViewModelFactory(private val forageableDoa: ForageableDao) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(ForageableViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return ForageableViewModel(forageableDoa) as T
        }
